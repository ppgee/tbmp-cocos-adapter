...
var cacheManager = {
  ...
  _cache() {
    ...
    for (var id in this.cacheQueue) {
      ...
      if (cacheBundleRoot) {
        localPath =
          `${this.cacheDir}/${cacheBundleRoot}/${time}${suffix++}${$global.cc.path.extname(id)}`;
      } else {
  ...
  clearLRU() {
    ...
    this.cachedFiles.forEach(function (val, key) {
      ...
      if (self._isZipFile(key) && $global.cc.assetManager.bundles.find(bundle =>
        bundle.base.indexOf(val.url) !== -1)) return;
      caches.push({
        originUrl: key,
        url: val.url,
