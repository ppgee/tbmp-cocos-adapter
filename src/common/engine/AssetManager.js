...
function handleZip(url, options, onComplete) {
  ...
  if (cachedUnzip) {
    ...
    downloadFile(url, null, options.header, options.onFileProgress, function (err,
      downloadedZipPath) {
      if (err) {
        onComplete && onComplete(err);
        return;
      }
      cacheManager.unzipAndCacheBundle(url, downloadedZipPath, options.__cacheBundleRoot__,
        onComplete);
    });
  } else {
  ...
  $global.cc.assetManager.transformPipeline.append(function (task) {
    ...
    for (var i = 0, l = input.length; i < l; i++) {
      ...
      if (!item.config) {
        ...
        options.cacheEnabled = options.cacheEnabled !== undefined ? options.cacheEnabled :
          false;
      } else {
